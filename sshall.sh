...
do
        ...
        ssh $username@${node[i]} -c "cat ~/.ssh/authorized_keys.tmp >> ~/.ssh/authorized_keys | awk '!a[$2]++' > /tmp/authorized_keys && mv /tmp/authorized_keys ~/.ssh/authorized_keys && chmod 600 ~/.ssh/authorized_keys"
